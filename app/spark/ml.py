    ...
    try:
            ...
            [
                {
                    "$group": {
                        ...
                        "checkout_events": {
                            "$sum": {
                                "$cond": [{"$eq": ["$event_type", "purchase"]}, 1, 0]
                            }
                        },
                        "cart_events": {
                            "$sum": {
        ...
        for u in user_data:
            ...
            avg_events_per_session = (
                total_events / sessions_count if sessions_count > 0 else 0
            )
        ...
        kmeans = KMeans(
            k=k_eff, seed=42, featuresCol="features", predictionCol="cluster"
        )
        ...
        results = predictions.select(
            "user_id", "cluster", "total_events", "conversion_rate", "cart_rate"
        ).collect()
        ...
        for stat in cluster_stats:
            ...
            clusters[cluster_id] = {
                ...
                "conversion_rate": round(float(stat["avg_conversion"]), 4),
                "cart_rate": round(float(stat["avg_cart_rate"]), 4),
            }
        ...
        if is_admin_request:
            ...
            result["message"] = (
                f"Admin user '{original_username}' - showing segmentation for ALL {len(user_data)} users"
            )
...
def ml_conversion_prediction_tree(username=None):
    ...
    try:
        ...
        for session in sessions:
            ...
            has_purchase = (
                1.0 if any(e.get("event_type") == "purchase" for e in events) else 0.0
            )
        ...
        try:
            ...
            dt = DecisionTreeClassifier(
                featuresCol="features", labelCol="label", maxDepth=5, maxBins=32
            )
        ...
        feature_importance = {
            name: round(float(imp), 4) for name, imp in zip(feature_names, importance)
        }

        # Collect some predictions
        sample_predictions = (
            predictions.select("session_id", "label", "prediction", "probability")
            .limit(10)
            .collect()
        )
...
def ml_pattern_mining_fpgrowth(username=None, limit=None, **kwargs):
    ...
    try:
        ...
        for session in sessions:
            session_id = session.get("session_id")
            events = list(
                events_col().find({"session_id": session_id}).sort("timestamp", 1)
            )
            ...
            if len(pages) >= 2:  # Need at least 2 items for patterns
                transactions.append(
                    (int(session["_id"].generation_time.timestamp()), pages)
                )
