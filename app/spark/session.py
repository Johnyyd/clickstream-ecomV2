...
def get_spark() -> SparkSession:
    ...
    if not _spark:
        _spark = (
            SparkSession.builder.appName("Clickstream Analytics")
            # Memory configuration
            .config("spark.sql.warehouse.dir", "spark-warehouse")
            .config(
                "spark.executor.memory", "6g"
            )  # Increased for complex clickstream analysis
...
def get_spark_session() -> Optional[SparkSession]:
    ...
    try:
        ...
        print(
            "[Spark] Analytics features will be disabled. Install Java 8/11 and set JAVA_HOME to enable."
        )
        return None
    except Exception as e:
